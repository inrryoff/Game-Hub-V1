...
# ============================================================
update() {
    local version_url="https://raw.githubusercontent.com/inrryoff/GameHub-PRO-X/main/script/boost.sh"
    local script_url="https://raw.githubusercontent.com/inrryoff/GameHub-PRO-X/main/version.txt"
    ...
    local remote_version=$(curl -s "$version_url" | tr -d '\r\n')

    if [ -z "$remote_version" ]; then
        echo "Não foi possível verificar a versão. Pulando..."
        return
    fi

    if [ "$remote_version" != "$VERSION" ]; then
        echo "Nova versão disponível: v$remote_version (Atual: v$_VERSION)"
        ...
        if curl -s -o "$tmp_script" "$script_url"; then
            ...
            echo "Atualizado para v$remote_version! Reiniciando..."
            su -c "sh $module_path"
            exit
        else
            echo "Falha no download da atualização."
        fi
    else
        echo "O script já está na versão mais recente (v$_VERSION)."
    fi
}
