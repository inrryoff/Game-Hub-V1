...
FONTE_RGB="$MODPATH/binarios/rgb.c"
...
install_dp() {
    ...
    say "${CYAN}   📦 CONFIGURANDO AMBIENTE PELA PRIMEIRA VEZ${NC}\n"

    if [ -d "/data/data/com.termux" ]; then
        say "${YELLOW}🛠️ Instalando clang e ncurses... (Aguarde)${NC}"
        pkg update -y && pkg install clang ncurses-utils -y > /dev/null 2>&1
        
        if [ $? -eq 0 ]; then
            say "${GREEN}✅ Ferramentas instaladas!${NC}"
        else
            say "${RED}❌ Falha ao baixar dependências. Verifique a internet.${NC}"
            return 1
        fi

        if [ -f "$FONTE_RGB" ]; then
            say "${YELLOW}🚀 Compilando motor ${RED}R${NC}${GREEN}G${NC}${BLUE}B${NC} do: $MODID${NC}"
            if clang -O3 "$FONTE_RGB" -lm -o "$PREFIX/bin/rgb"; then
                chmod +x "$PREFIX/bin/rgb"
                say "${GREEN}✅ Motor RGB pronto para uso!${NC}"
            else
                say "${RED}❌ Erro na compilação.${NC}"
                return 1
            fi
        fi
    fi
    ...
    touch "$CHECK_DP"
    say "${GREEN}✨ Tudo pronto! Reiniciando para aplicar os efeitos...${NC}"
    sleep 2
}
...
_spinner_anim() {
    ...
    local cores=($(rgb --spin 2>/dev/null))
    [ ${#cores[@]} -eq 0 ] && cores=(31 32 33 34 35 36)
    ...
    while kill -0 "$pid" 2>/dev/null; do
        for frame in "${frames[@]}"; do
            local cor="${cores[$idx % n_cores]}"
            printf "\r\e[K\e[%sm[%s]\e[0m \e[33m%s\e[0m" "$cor" "$frame" "$msg"
            idx=$((idx + 1))
            sleep "$delay"
