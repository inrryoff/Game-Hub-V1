...
modo_normal() {
    ...
    say "${YELLOW}⚙️ Restaurando CPU governor...${NC}"
    for policy in /sys/devices/system/cpu/cpufreq/policy*; do
        if [ -f "$policy/scaling_governor" ]; then
            if grep -q "schedutil" "$policy/scaling_available_governors" 2>/dev/null; then
                echo "schedutil" > "$policy/scaling_governor"
            elif grep -q "interactive" "$policy/scaling_available_governors" 2>/dev/null; then
                echo "interactive" > "$policy/scaling_governor"
            fi
        fi

        if [ -f "$policy/cpuinfo_min_freq" ] && [ -f "$policy/scaling_min_freq" ]; then
            cat "$policy/cpuinfo_min_freq" > "$policy/scaling_min_freq" 2>/dev/null
        fi
    done
    say "${GREEN}✓ CPU governor restaurado${NC}"
